...
```{r fig.cap= "Scatter and QQplots demonstrating the 4 assumtions of a linear model"}
par(mfrow = c(1, 2))
...
qqplot(batting$DOUBLE, batting$RUNS, xlab = "Doubles", ylab = "Runs", main = "QQ-Plot")
```
...
**- Normality:** On inspection, our QQ plot demonstrates the fact that the data is normally distributed.
...
We are 95% confident that the mean runs per player is between 184.75 and 228.45.
...
We are 95% confident that the mean number of runs for a player with 200 doubles is between 92.15 and 135.85.

# Fit of Model

The $R^2$ value of our model is 0.9551 and the adjusted $R^2$ is 0.9550. This means that the model explains 95.51% of the variance of the recorded events. Additionally, the residual plot in figure 2 shows how the data points share a similar spread which implies that the model is a good fit. As a result, we conclude that this model fits our data well and explains the majority of the variation in the data.
...
plot(fitted(model), res, ylab = "Residuals", xlab = "Fitted Model", main = "Residual Plot")
abline(0,0)
```
